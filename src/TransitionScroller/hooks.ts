      ...
      if (ref.current) {
        ...
        if ((!completeHide && dy > 0) || dh < ref.current.offsetHeight) {
          ...
          if (moveOutPropotion >= 1) {
            ...
            ref.current.style[styleKey] = interpolateFn(
              moveOutPropotion > 0 ? moveOutPropotion : 0
            );
          }
        }
...
const calcTransitionParams = (
  inputs: UseStickyTransition = {}
): UseStickyScroll => {
  const interpolateFn =
    inputs.interpolateFn || ((ratio: number) => `translateY(${-ratio * 100}%)`);
  return {
    interpolateFn,
...
export const useStickyTransition = (inputs?: UseStickyTransition) => {
  ...
  const onTransitionEnd = React.useCallback(() => {
    if (ref.current) {
      ref.current.classList.remove(params.current.transitionClass);
    }
  }, [ref]);
...
const calcAnimationParams = (
  inputs: UseStickyAnimation = {}
): UseStickyScroll => {
  const interpolateFn =
    inputs.interpolateFn || ((ratio: number) => `${ratio - 1}s`);
  return {
    interpolateFn,
    transitionClass:
      inputs.transitionClass ||
      'animate-[750ms_ease-in-out_0s_1_reverse_forwards_running_scroll-out]',
    hideStyle: interpolateFn(1),
    showStyle: interpolateFn(0),
